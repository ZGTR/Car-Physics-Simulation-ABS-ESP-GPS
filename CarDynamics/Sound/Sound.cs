...
namespace Sounds
{
    ...
    class Sound
    {
        ...
        static float MoveFactorPerSecond
        {
            get 
            {
                float moveFactor = elapsedTimeThisFrameInMs / 1000.0f;               
        ...
        static float elapsedTimeThisFrameInMs;
            ...
            string gearSound, float volume, float pitch)
        {
            ...
            gearsCategory.SetVolume(MathHelper.Clamp(volume, 0, 1) * 0.8f);
        ...
        /// </summary>
        /// 
        public static float MeterPerSecToMph =
           1.609344f * ((60.0f * 60.0f) / 1000.0f),
           MphToMeterPerSec = 1.0f / MeterPerSecToMph;

        static float maxPossibleSpeed = 270.0f * MphToMeterPerSec;

        public static void UpdateGearSound(float speed, float acceleration)//,int newGear)
        {
            // Calculate new gear depending on the current speed
            int newGear = (int)(NumberOfGears * speed / maxPossibleSpeed);
            Console.WriteLine(speed.ToString());
            acceleration /= 3;
            if (acceleration > 1)
                acceleration = 1;
            else
                if (acceleration < -0.025)
                    acceleration = -0.025f;


            if (Keyboard.GetState().IsKeyDown(Keys.F))
            {
                Console.WriteLine("Gear" + newGear.ToString());
                Console.WriteLine("speed" + speed.ToString());
                Console.WriteLine("acc" + acceleration.ToString());
            }
            ...
            if (speed < 0)
                speed = MathHelper.Clamp(
                    Math.Abs(speed), 0, maxPossibleSpeed / 5);
            float gearPercentage = (float)
                ((int)((speed / maxPossibleSpeed) * 499) %
        ...
        {
            if (brakeSoundStillPlayingMs > 0)
                brakeSoundStillPlayingMs -= elapsedTimeThisFrameInMs;
            if (crashSoundStillPlayingMs > 0)
                crashSoundStillPlayingMs -= elapsedTimeThisFrameInMs;
